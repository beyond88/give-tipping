(function($) {
    ...
    let maxField = 4; //Input fields increment limitation
	let wrapper = $('#gt-append_body'); //Input field wrapper
	const total = ($('.gt-playfield').length);
	let counter = 1;
	let currentFields = total;

	$(document).on('click', ".gt-click-overlay", function () { //Once add button is clicked

		let total = $('.gt-playfield').length;
	...
	$(document).on('click', '.gt-remove-button', function () {
